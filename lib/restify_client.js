...
function clientForService(service, options, callback) {
	...
	if (_this.useDns) {
		if (_this.services[service].clients['dns'] === undefined) {
			...
			if (_this.dnsPrefix !== null) {
				path = `${_this.dnsPrefix}${path}`;
			}
			let addressObj = {
				address: path,
				port: _this.defaultPort
			};
			_this.services[service].clients['dns'] = _this.buildClient(addressObj, restifyOptions);
		}
		return cb(null, _this.services[service].clients['dns']);
	} else {
