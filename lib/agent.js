...
function sendPass(callback) {
	...
	return _this.getConsulClient().agent.check.pass(_this.service.identifier, function(err) {
		...
		if (err.message === 'CheckID does not have associated TTL') {
			return _this.registerService(callback);
		} else {
			return callback(err);
		}
	});
...
function deregister(callback) {
	...
	return async.series([
		...
		function(doneCallback) {
			_this.getConsulClient().agent.service.deregister(_this.service.identifier, doneCallback);
		}
	], function(err) {
		return callback(err);
	});
...
function stop(callback) {
	...
	if (this.nextTimerHandle !== null) {
		clearTimeout(nextTimerHandle);
		this.nextTimerHandle = null;
	}
...
function getCache() {
	if (this.cache === undefined) {
		this.cache = addressCache.buildCache({
